...
function mouseClicked() {
  if (mouseX >= 0 && mouseX <= width && mouseY >= 0 && mouseY <= height) {
    ...
    console.log("col: " + floor(mouseX / resolution) + ", row:" + floor(mouseY / resolution));
    let col = floor(mouseX / resolution);
    let row = floor(mouseY / resolution);
    if (col < cellGrid.grid.length && row < cellGrid.grid[col].length) {
      if (cellGrid.grid[col][row] == null)
        cellGrid.grid[col][row] = new Cell();
      else
        cellGrid.grid[col][row] = null;
    }
